...
export default {
    data:() => ({
        ...
        columns: [
            { name: "eduId", header: "No", align: "center", minWidth:100},
            { name: "eduRegDt", header: "등록일자", align: "center",minWidth:100 },
            { name: "typeName", header: "교육유형", align: "left",minWidth:200 },
            { name: "targetNameList", header: "교육대상", align: "left",minWidth:300 },
            { name: "time", header: "교육시간", align: "center",minWidth:150 },
            { name: "minStudentCount", header: "최소인원", align: "center",minWidth:100 },
            { name: "status", header: "진행상태", align: "center",minWidth:100},
        ],
        theme: 'clean',
        options:{
            pageOptions:{
    ...
    methods:{
        ...
        searchListByDateRange: function() {
            const _search = this.date;
            if(_search){
                if(_search.length != 2){
                    alert('기간을 선택해주세요');
                    return; 
                }
                const _datesArray = this.date.sort();
                this.searchDate = { startDate : _datesArray[0], endDate : _datesArray[1]};
            } 
            this.$refs.tuiGrid.invoke('readData',1,this.searchDate,true);
        },
